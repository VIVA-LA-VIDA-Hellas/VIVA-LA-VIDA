...
POST_TURN_GRACE_GREEN_S = 0.25  # Ignore green box triggers for this long after turn
POST_TURN_GRACE_RED_S   = 0.25 # Ignore red box triggers for this long after turn
...
obstacle_present = False       # True if we've seen red/green recently
obstacle_clear_streak = 0      # how many consecutive frames without red/green
...
try:
    ...
    while True:
        ...
        # ---- Obstacle presence hysteresis (red/green) ----
        any_obstacle_seen = red_seen or green_seen

        if any_obstacle_seen:
            # we see a red/green obstacle right now -> latch presence
            obstacle_present = True
            obstacle_clear_streak = 0
        else:
            # no obstacle this frame; count how many clear frames in a row
            if obstacle_present:
                obstacle_clear_streak += 1
                if obstacle_clear_streak >= OBSTACLE_CLEAR_FRAMES:
                    # only now do we "forget" the obstacle
        ...
        # force obstacle priority if its area is large (close)
        if boxes:
            biggest_area = max(b[1] for b in boxes)
            if biggest_area > 6000:   # tune
        ...
        if (not turn_active) and (not in_blue_backward) and (not post_turn_back_mode) and (not emergency_mode):
            ...
            if not avoidance_mode:
                ...
                #if not line_closer:   # <-- this is the only new guard
                if obstacle_closer or any(boxes_intersect(car_box, b[2]) for b in boxes):
                    line_closer = False
                if not line_closer:
            ...
            if avoidance_mode:
                ...
                if elapsed < AVOID_BACK_DURATION:
                    ...
                    if (not any(boxes_intersect(car_box, b[2]) for b in boxes)) and (not obstacle_present):
                        avoidance_mode = False
                        avoid_direction = None
                        state = "normal"
                        set_run_state("cruise")
