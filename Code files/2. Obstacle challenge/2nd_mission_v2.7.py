...
BLUE_BACK_DURATION    = 1.0  # how long to reverse in blue-backward mode
POST_BACK_FOLLOW_S    = 1.0  # how long to bias steering toward box after back
...
TURN_MOTOR_SPEED      = 14   # Motor speed while performing a 90° line-based turn
...
LEFT_COLLIDE_ANGLE = 105    # Steering angle on left side collision correction
RIGHT_COLLIDE_ANGLE = 75    # Steering angle on right side collision correction
...
FRONT_TRIGGER = 70                 # FRONT distance to allow a turn
...
GREEN_LO   = np.array([70, 100, 70],  dtype=np.uint8)
GREEN_HI   = np.array([90, 225, 155], dtype=np.uint8)
ORANGE_LO  = np.array([10,  155, 180], dtype=np.uint8)
...
TIME_TO_STOP = 4.5  # seconds to drive straight before final stop
...
turn_count = 0
lap_count  = 1
finish_start_time = None  # for final straight timing
...
try:
    ...
    while True:
        ...
        # --- Crop masks to active ROI ---
        mask_red_roi   = mask_red[y1:y2, x1:x2]
        mask_green_roi = mask_green[y1:y2, x1:x2]

        # --- RED detection in cropped area ---
        red_candidate = thin_shape_reject(
            get_largest_contour(mask_red_roi, min_area=MIN_AREA)
        )
        if red_candidate:
            cnt, tl_roi, br_roi, area = red_candidate
            rx1, ry1 = tl_roi
            rx2, ry2 = br_roi

            # Map ROI coords -> full-frame coords
            tl_full = (rx1 + x1, ry1 + y1)
            br_full = (rx2 + x1, ry2 + y1)

            red_data = (cnt, tl_full, br_full, area)
            boxes.append(("Red", area, (*tl_full, *br_full)))

        # --- GREEN detection in cropped area ---
        green_candidate = thin_shape_reject(
            get_largest_contour(mask_green_roi, min_area=MIN_AREA)
        )
        if green_candidate:
            cnt, tl_roi, br_roi, area = green_candidate
            gx1, gy1 = tl_roi
            gx2, gy2 = br_roi

            tl_full = (gx1 + x1, gy1 + y1)
            br_full = (gx2 + x1, gy2 + y1)

            green_data = (cnt, tl_full, br_full, area)
            boxes.append(("Green", area, (*tl_full, *br_full)))
        ...
        # (optional) draw it on the debug image so you can see the box
        cv2.rectangle(
            img_bgr,
            (car_box[0], car_box[1]),
            (car_box[2], car_box[3]),
        ...
        if line_trigger_raw and line_y_for_turn >= 0 and closest_obstacle_y >= 0:
            # Both visible -> whoever is lower in the image (bigger y) is closer
            if closest_obstacle_y > line_y_for_turn + OBSTACLE_LINE_MARGIN_PX:
                ...
                # Similar distance → prefer line or obstacle, your choice.
                # If you want line to win ties:
                line_closer = True
        ...
        if chosen_box is not None or line_trigger_raw:
            if chosen_box is not None:
                x1, y1, x2, y2 = chosen_box
                cx = 0.5 * (x1 + x2)
                off_norm = (cx - center_x) / float(center_x)
            else:
                cx = None
                off_norm = 0.0
        ...
        if fsm_state == STATE_CRUISE:
            ...
            if (chosen_color is not None and
                ...
                if area_ok and y_ok:
                    ...
                    # >>> RESET YAW when starting avoidance <<<
                    with yaw_lock:
                        yaw = 0.0
                        current_yaw = 0.0
            ...
            if abs(current_yaw) >= TURN_FAILSAFE_MAX_DEG:
                ...
                dbg(
                    f"TURN end: dir={turn_dir}, yaw={current_yaw:.1f}, "
                    f"turn_count={turn_count}, lap={lap_count}"
                )
