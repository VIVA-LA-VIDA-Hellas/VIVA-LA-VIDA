...
TURN_MOTOR_SPEED      = 15   # Motor speed while performing a 90Β° line-based turn
...
MIN_AREA          = 2800    # Minimum contour area to accept as a box
MAX_AREA          = 18000   # Area at which box is considered "very close"
COLOR_HOLD_FRAMES = 2       # Frames the same color must persist to be β€lockedβ€
...
XPOS_MAX_OFFSET = 0.8    # clamp |offset|, treat anything beyond as 0.7
...
POST_TURN_LINE_IGNORE_S  = 5.0    # Time to ignore blue/orange lines after backing (sec)
...
LINE_ORIENT_MIN_DEG   = 25    # Minimum angle (deg) to accept a line as β€diagonalβ€
LINE_ORIENT_MAX_DEG   = 65    # Maximum angle (deg) to accept a line as β€diagonalβ€
LINE_MASK_THICKNESS   = 9     # Thickness of mask drawn over detected line band

# ---- Turn-related constants ----
LINE_CENTER_BLUE_Y_MIN     = 240  # Minimal Y for blue line to be valid for turn
LINE_CENTER_ORANGE_Y_MIN   = 240  # Minimal Y for orange line to be valid for turn
...
RED1_LO    = np.array([0,   130, 120], dtype=np.uint8)  # Red lower hue range 1
RED1_HI    = np.array([10,  255, 190], dtype=np.uint8)  # Red upper hue range 1
RED2_LO    = np.array([0,   130, 100], dtype=np.uint8)  # Red lower hue range 2
RED2_HI    = np.array([10,  255, 175], dtype=np.uint8)  # Red upper hue range 2
GREEN_LO   = np.array([70, 130, 65],  dtype=np.uint8)   # Green lower HSV bound
GREEN_HI   = np.array([85, 230, 130], dtype=np.uint8)   # Green upper HSV bound
ORANGE_LO  = np.array([6,  160, 180], dtype=np.uint8)   # Orange lower HSV bound
ORANGE_HI  = np.array([14, 200, 210], dtype=np.uint8)   # Orange upper HSV bound
BLUE_LO    = np.array([110, 140, 110], dtype=np.uint8)  # Blue lower HSV bound
BLUE_HI    = np.array([130, 220, 250], dtype=np.uint8)  # Blue upper HSV bound
...
def reset_yaw_listener():
    global yaw
    while True:
        input("Press ENTER to reset yaw to 0Β°")
        with yaw_lock:
            yaw = 0.0
            print("Yaw reset to 0Β°")
...
# ===============================
# SMART UNPARK (two-phase; faster; left-case phase 2 = 65Β° + extra ~60Β° left if direction == "left")
# ===============================
...
try:
    ...
    while True:
        ...
        # Choose the β€closestβ€ line Y for comparison with obstacle
        valid_ys = []
        ...
        # Which is β€closerβ€: line or obstacle?
        ...
        if fsm_state == STATE_CRUISE:
            ...
            # Base IMU keep-straight (strong gains β€“ no obstacle locked)
            ...
            if (chosen_color is not None and
                ...
                set_run_state(f"avoid lock β€“ {obstacle_lock_color.lower()}")
            ...
            # ---- Otherwise: line-based turn (if allowed) ----
                ...
                if blue_trigger and not orange_trigger:
                    set_run_state(f"blue line β€“ turn {turn_dir.lower()}")
                elif orange_trigger and not blue_trigger:
                    set_run_state(f"orange line β€“ turn {turn_dir.lower()}")
                else:
                    set_run_state(f"line β€“ turn {turn_dir.lower()}")
            ...
            if back_cm >= POST_TURN_BACK_CLEAR_CM or elapsed_back >= POST_TURN_BACK_TIMEOUT_S:
                ...
                # Done backing up β€“ resume cruise
                set_motor_speed(MOTOR_FWD, MOTOR_REV, NORMAL_SPEED)
                fsm_state        = STATE_CRUISE
                state_start_time = now
                settle_until_ts  = time.time() + SETTLE_DURATION
            ...
            if avoid_phase == "back_off":
                ...
                if locked_seen and chosen_box is not None:
                    # Obstacle still visible β€“ follow around it
                    area       = chosen_area
                    ...
                    # Locked obstacle not seen this frame β€“ drive roughly straight
                    target_angle = imu_center_servo(
                        current_yaw,
