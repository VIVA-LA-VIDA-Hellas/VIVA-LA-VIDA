...
AVOID_SPEED           = 12   # Reverse speed when doing obstacle-avoidance backup
...
MIN_AREA          = 1900    # Minimum contour area to accept as a box
MAX_AREA          = 25000   # Area at which box is considered "very close"
...
RED1_LO    = np.array([2,   200, 140], dtype=np.uint8)  # Red lower hue range 1
RED1_HI    = np.array([5,  250, 160], dtype=np.uint8)  # Red upper hue range 1
RED2_LO    = np.array([170, 190, 110], dtype=np.uint8)  # Red lower hue range 2
RED2_HI    = np.array([190, 240, 160], dtype=np.uint8)  # Red upper hue range 2
GREEN_LO   = np.array([70, 170, 90],  dtype=np.uint8)   # Green lower HSV bound
GREEN_HI   = np.array([80, 230, 110], dtype=np.uint8)   # Green upper HSV bound
...
POST_TURN_GRACE_GREEN_S = 0.1  # Ignore green box triggers for this long after turn
POST_TURN_GRACE_RED_S   = 0.1 # Ignore red box triggers for this long after turn
...
reverse_mode = False
reverse_start_time = 0.0
REVERSE_DURATION = 0.6   # seconds


NEAR_BOX_PIXELS = 50  # how many pixels above car_box we consider "too close"

def box_close_not_touching(box_coords, car_box, margin_px=NEAR_BOX_PIXELS):
    x1, y1, x2, y2 = box_coords
    car_x1, car_y1, car_x2, car_y2 = car_box

    # vertically: box bottom inside [car_y1 - margin, car_y1)
    vertically_close = (y2 >= car_y1 - margin_px) and (y2 < car_y1)

    return vertically_close and (not boxes_intersect(car_box, box_coords))
...
# post-turn backward reposition state
post_turn_back_mode = False
post_turn_back_start_time = 0.0
...
try:
    ...
    while True:
        ...
        if reverse_mode:
            ...
            continue   # skip rest of loop while reversing


        if (not turn_active
            and out_of_cooldown
            and out_of_min_interval
            and not emergency_mode
            and not post_turn_back_mode
            and (time.time() >= post_turn_line_ignore_until)):
        ...
        # --- POST-TURN BACKWARD REPOSITION ------------------------------------
        if post_turn_back_mode:
            # keep wheels straight while backing
            set_servo_angle(SERVO_CHANNEL, CENTER_ANGLE)
            set_motor_speed(MOTOR_FWD, MOTOR_REV, -POST_TURN_BACK_SPEED)

            b_cm_back = tof_cm(sensors["back"])
            elapsed_back = time.time() - post_turn_back_start_time

            # Debug (optional):
            # print(f"[POST-TURN BACK] back={b_cm_back:.1f}cm, t={elapsed_back:.2f}s")

            if b_cm_back >= POST_TURN_BACK_CLEAR_CM or elapsed_back >= POST_TURN_BACK_TIMEOUT_S:
                # done backing up – treat this as the real 'turn end'
                post_turn_back_mode = False

                last_turn_end_time = time.time()
                next_turn_allowed_time = last_turn_end_time + TURN_COOLDOWN_SEC

                # ignore blue/orange lines for a short period so we don't re-trigger
                post_turn_line_ignore_until = last_turn_end_time + POST_TURN_LINE_IGNORE_S
                ...
                set_run_state("cruise")
                # fall through to normal processing this frame
            else:
                # stay in reverse; skip the rest of the logic for this frame
        ...
        if (line_trigger
            and TURN_FRONT_MIN_CM <= f_ultra <= TURN_FRONT_MAX_CM
            and not post_turn_back_mode
            and (time.time() >= post_turn_line_ignore_until)):
            blue_gate_streak += 1
        else:
        ...
        # Determine if the line really has priority:
        #   - it is closer (line_closer),
        #   - turn side is decided (Turn != "No"),
        #   - we are actually allowed to start a turn (ok_to_turn + ToF gate).
        line_has_priority = (
            line_closer
            and (Turn in ("Left", "Right"))
            and ok_to_turn
        ...
        # ---- Obstacle vs line "who is closer?" priority metric ----
        # For boxes, use the bottom of the bounding box (y2). Larger y2 = closer to robot.
        closest_obstacle_y = -1
        if boxes:
            closest_obstacle_y = max(b[2][3] for b in boxes)  # b[2] = (x1, y1, x2, y2), index 3 = y2

        obstacle_closer = False
        line_closer = False

        if line_trigger and line_y_for_turn is not None and line_y_for_turn >= 0 and closest_obstacle_y >= 0:
            # Both line and obstacle visible: compare their bottom Y with a margin
            if closest_obstacle_y > line_y_for_turn + OBSTACLE_LINE_MARGIN_PX:
                # Obstacle is clearly closer
                obstacle_closer = True
            elif line_y_for_turn > closest_obstacle_y + OBSTACLE_LINE_MARGIN_PX:
                # Line is clearly closer
                line_closer = True
            else:
                # Roughly same distance -> prefer line by default
                line_closer = True
        elif line_trigger and (closest_obstacle_y < 0):
            # Only line visible
            line_closer = True
        elif (not line_trigger) and (closest_obstacle_y >= 0):
            # Only obstacle visible
            obstacle_closer = True
        ...
        if (Turn in ("Left", "Right")
            ...
            and ok_to_turn
            and tof_line_turn_gate
            and not obstacle_closer):

            # --- PRIORITY: line turn overrides avoidance ---
            avoidance_mode = False
            avoid_direction = None
            state = "normal"   # ensure normal forward logic works after turn
        ...
        if turn_active:
            ...
            if failsafe_yaw:
                ...
                if turn_count >= TOTAL_TURNS:
                    print("[LAP] All 3 laps completed. Driving straight then stopping.", flush=True)
                ...
                # ====== START POST-TURN BACKWARD REPOSITION (more laps remaining) ======
                post_turn_back_mode = True
                post_turn_back_start_time = time.time()

                if first_turn_gate_active:
                    first_turn_gate_active = False

                set_run_state("post-turn reverse")
                # immediately start reversing with centered wheels
                set_servo_angle(SERVO_CHANNEL, CENTER_ANGLE)
                set_motor_speed(MOTOR_FWD, MOTOR_REV, -POST_TURN_BACK_SPEED)
        ...
        # and NOT in the post-turn reverse phase.
        if (not turn_active) and (not in_blue_backward) and (not post_turn_back_mode) and (not emergency_mode):

            # --- Enter avoidance mode if needed ---
            if not avoidance_mode:
                now_ts = time.time()
                # If a line is clearly closer, let the TURN logic handle it
                if not line_has_priority:
                    for color, _, box_coords in boxes:
                        # 1) NORMAL CASE: box already inside car_box → existing behavior
                        ...
                        # 2) EARLY AVOID: too close but NOT yet touching car_box
                        elif box_close_not_touching(box_coords, car_box):
                            x1, y1, x2, y2 = box_coords
                            box_center_x = (x1 + x2) // 2

                            # RED on the LEFT side of the image → go back and turn RIGHT
                            if color == "Red" and box_center_x < center_x:
                                avoidance_mode = True
                            ...
                            # GREEN on the RIGHT side of the image → go back and turn LEFT
                            if color == "Green" and box_center_x >= center_x:
                                avoidance_mode = True
            ...
            if avoidance_mode:
                ...
                if elapsed < AVOID_BACK_DURATION:
                    set_servo_angle(
                        SERVO_CHANNEL,
                        RIGHT_NEAR if avoid_direction == "right" else LEFT_NEAR
                    )
                    set_motor_speed(MOTOR_FWD, MOTOR_REV, -AVOID_SPEED)
                else:
                    set_motor_speed(MOTOR_FWD, MOTOR_REV, NORMAL_SPEED)
        ...
        # --- High-level perception flags (RUN EVERY FRAME) ---
        any_boxes = (len(boxes) > 0)
        any_line  = line_trigger  # from earlier line decision logic

        # Cruise is only allowed when:
        # - no obstacle boxes,
        # - no line asking for a turn,
        # - and no special modes are active.
        cruise_allowed = (
            (not any_boxes)
            and (not any_line)
            and (not avoidance_mode)
        ...
        # ==== NORMAL FORWARD + IMU STRAIGHTENING / OBSTACLE FOLLOW =====
        if not avoidance_mode and not in_blue_backward and not turn_active and state == "normal" and not emergency_mode:
            motor_speed = NORMAL_SPEED
            set_motor_speed(MOTOR_FWD, MOTOR_REV, motor_speed)

            # ---------- PURE CRUISE MODE ----------
            # No obstacles, no line wanting a turn, no special modes
            if cruise_allowed:
                set_run_state("cruise")

                # Strong IMU straightening only in true cruise
                target_angle = imu_center_servo(
                    current_yaw,
                ...
                # Side-wall collision protection (ultrasonic)
                l = l_ultra
                r = r_ultra
                ...
                # If path is clear and we're calm, aggressively decay yaw drift
            ...
            # ---------- OBSTACLE MODE (box present) ----------
            elif any_boxes:
                # We are in "obstacle mode": do NOT call cruise IMU centering.
                # Steering is dominated by the obstacle; yaw is only a helper.
                ...
                if frame_count >= COLOR_HOLD_FRAMES:
                    # lock onto this obstacle color and stay in this mode
                    if chosen_color == "Red":
                        set_run_state("red obstacle avoid")
                    elif chosen_color == "Green":
                        set_run_state("green obstacle avoid")
                    else:
                        set_run_state(f"{chosen_color.lower()} obstacle avoid")

                    # area → steering angle
                    area_angle = compute_servo_angle(chosen_color, chosen_area)

                    # box-based yaw gain: stronger when box is closer
                    ...
                    # final obstacle steering (IMU term only biases, doesn't fight it)
                    if chosen_color == "Red":
            ...
            # ---------- NON-CRUISE, NO BOX (e.g. line seen but not turning yet) ----------
            else:
                # No box, but also not allowed to be in pure cruise (e.g. line nearby).
                # Just keep IMU centering but don't print "cruise".
                set_run_state("pre-turn / approach")
                target_angle = imu_center_servo(
                    current_yaw,
                    YAW_DEADBAND_DEG_BASE,
                    YAW_KP_BASE,
        ...
        draw_boxes(img_bgr, mask_red,   (0, 0, 255),   "Red")
        draw_boxes(img_bgr, mask_green, (0, 255, 0),   "Green")

        # Draw lines (orange/blue)
        draw_lines(img_bgr, mask_orange, (0, 165, 255), "Orange")
        draw_lines(img_bgr, mask_blue,   (255, 0, 0),   "Blue")

        # Show
        cv2.imshow("Camera annotated", img_bgr)
