...
# v5.8-perf (optimized for lower latency with ultrasonics)
# -----------------------------------------------------------------------------------------------------------------------
...
# NOTE: Removed numpy to avoid heavy dependency in hot path
...
MAX_TURN_ANGLE = 120          # Maximum yaw change (degrees) to force stop turn

# ---------- Laps ----------
MAX_LAPS = 3                  # Maximum number of laps before stopping (0 = unlimited)
POST_LAP_DURATION = 1         # Time to drive forward after final lap before stopping (seconds)

# ---------- Sensor data filtering ----------
N_READINGS = 1                # keep light: median window length
US_QUEUE_LEN = 1              # keep gpiozero internal queue short to avoid double filtering
FILTER_ALPHA = 0.3            # Ultrasonic EMA: 0.1 smoother, 0.5 faster reaction
FILTER_JUMP = 40              # Ultrasonic maximum jump (cm) allowed between readings
FILTER_ALPHA_TOF = 1.0        # ToF (unused when USE_TOF_* == 0)
FILTER_JUMP_TOF = 999
US_MAX_DISTANCE_FRONT = 3.43  # Ultrasonic front max read distance (meters)
US_MAX_DISTANCE_SIDE = 1.72   # Ultrasonic side max read distance (meters)

# ---------- Loop timing ----------
LOOP_DELAY = 0.005            # ~200 Hz FSM
SENSOR_DELAY = 0.010          # ~100 Hz sensor reader

# ---------- Actuator I/O throttling ----------
SERVO_MIN_UPDATE_DT = 1/50.0  # cap servo writes ~50 Hz
SERVO_MIN_STEP = 0.5          # ignore micro step changes < 0.5°
MOTOR_MIN_UPDATE_DT = 1/40.0  # cap motor writes ~40 Hz
...
def median_of(vals):
    """Small-list median without numpy (vals may include None)."""
...
# --------- Ultrasonic sensors ----------
# (Optionally enable pigpio for better timing jitter)
# from gpiozero import Device
# from gpiozero.pins.pigpio import PiGPIOFactory
...
COLLECT_FOR_GUI = bool(USE_GUI)

# ===============================
# GYRO THREAD (decoupled I²C)
# ===============================
yaw_lock = threading.Lock()
shared_yaw_deg = 0.0
GYRO_HZ = 200  # 150–200 Hz is plenty

def gyro_thread():
    global shared_yaw_deg
    ...
    while True:
        ...
        z_f = ALPHA * raw_z + (1 - ALPHA) * z_prev
        z_prev = z_f
        with yaw_lock:
            shared_yaw_deg += (z_f * RAD2DEG) * dt
        # pace the thread
        remaining = (1.0 / GYRO_HZ) - (time.perf_counter() - now)
        if remaining > 0:
...
class RobotController:
    def __init__(self, pca):
        ...
        self._last_servo_send = time.perf_counter()
        self._last_motor_send = time.perf_counter()
        self._last_motor_dc_fwd = -1
        self._last_motor_dc_rev = -1
    ...
    def set_servo(self, angle):
        ...
        # 1b) ignore tiny steps and rate-limit outbound PWM
        now = time.perf_counter()
        if abs(target - self._servo_last_angle) < SERVO_MIN_STEP:
            return self._servo_last_angle
        if (now - self._last_servo_send) < SERVO_MIN_UPDATE_DT:
            return self._servo_last_angle
...
def robot_loop():
    ...
    direction = None
    # yaw handled by gyro thread; local mirror for convenience
    with yaw_lock:
        yaw = shared_yaw_deg
    turn_start_yaw = 0.0
    turn_start_time = 0.0
    ...
    # loop rate debug
    if not hasattr(robot_loop, "_acc"):
        robot_loop._acc = 0.0; robot_loop._n = 0; robot_loop._t0 = time.perf_counter()
    ...
    while True:
        ...
        # loop rate meter (prints only when DEBUG=1)
        robot_loop._n += 1
        if (time.perf_counter() - robot_loop._t0) > 1.0:
            hz = robot_loop._n / (time.perf_counter() - robot_loop._t0)
            dprint(f"[loop] {hz:.1f} Hz")
            robot_loop._n = 0; robot_loop._t0 = time.perf_counter()
        ...
        # ----------------------------
        # Sensor reads & gyro (yaw from shared thread)
        # ----------------------------
        with sensor_lock:
        ...
        # get yaw snapshot from gyro thread
        with yaw_lock:
            yaw = shared_yaw_deg
        ...
        if robot_loop._narrow_mode != prev_mode:
            ...
            if sum_lr is not None:
                dprint(f"[corridor] {mode_txt} (L+R = {sum_lr:.1f} cm)")
            else:
                dprint(f"[corridor] {mode_txt}")
        ...
        # If loop_event is False, do NOT run FSM/motors yet—just update plots
        ...
        if state == RobotState.IDLE:
            ...
            if turn_count >= 1:
                ...
                robot.set_servo(desired_servo_angle)  # Apply the desired servo angle
            ...
            if LOCK_TURN_DIRECTION == 1:
                if (globals().get('locked_turn_direction', None)) is None:
                    # capture first allowed direction
                    globals()['locked_turn_direction'] = proposed_direction
                    dprint(f"Turn direction locked to {globals()['locked_turn_direction']}")
                elif proposed_direction != globals()['locked_turn_direction']:
                    # keep waiting at TURN_INIT speed for the locked direction to open
                    status_text = f"Ignoring {proposed_direction} (locked to {globals()['locked_turn_direction']})"
                    robot.set_servo(desired_servo_angle)
                    time.sleep(LOOP_DELAY)
                    continue
                direction = globals()['locked_turn_direction']
            else:
            ...
            # start turning
            angle_cmd = TURN_ANGLE_LEFT if direction == "LEFT" else TURN_ANGLE_RIGHT
            robot.set_servo(angle_cmd)
            ...
            with yaw_lock:
                yaw_now = shared_yaw_deg
            turn_angle = yaw_now - turn_start_yaw  # +left, -right
            ...
            if stop_condition:
                ...
                # Snap yaw to nearest axis for consistency
                with yaw_lock:
                    snapped = snap90(shared_yaw_deg)
                    shared_yaw_deg = snapped
                    yaw = shared_yaw_deg
                ...
                if turn_count % 4 == 0:
                    ...
                    # if reached max laps -> do final drive then stop
            ...
            # --- User or laps stop: stay stopped until user action ---
            status_text = "Stopped"
            time.sleep(LOOP_DELAY)
            continue
...
def start_readings():
    ...
    # Start gyro thread once
    if not hasattr(start_readings, "gyro_thread_started"):
        threading.Thread(target=gyro_thread, daemon=True).start()
        start_readings.gyro_thread_started = True
...
if __name__ == "__main__":
    ...
    if USE_GUI:
        ...
        # Start the sensor reading & gyro threads
        GPIO.output(RED_LED, GPIO.HIGH)
        GPIO.output(GREEN_LED, GPIO.LOW)

        if 'sensor_thread' not in globals():
            sensor_thread = threading.Thread(target=sensor_reader, daemon=True)
            sensor_thread.start()
        if 'gyro_thr' not in globals():
            gyro_thr = threading.Thread(target=gyro_thread, daemon=True)
            gyro_thr.start()
