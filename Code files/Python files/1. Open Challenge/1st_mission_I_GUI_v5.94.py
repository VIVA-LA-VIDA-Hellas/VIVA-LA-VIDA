...
US_MAX_DISTANCE_FRONT = 2.8  # Ultrasonic front max read distance
...
def launch_gui(): #Initialize Tkinter GUI, matplotlib plots, sliders, and status indicators.
    ...
    def update_plot():
        ...
        if time_data:
            ...
            if len(front_left_data) > 0:
                ...
                ax_side.text(x, y_smooth, f"FL: {y_target:.1f} cm", color="magenta", fontsize=9, fontweight="bold", va="bottom", ha="left")
            # ----------------------------
            ...
            if len(front_right_data) > 0:
                ...
                ax_side.text(x, y_smooth, f"FR: {front_right_data[-1]:.1f} cm", color="red", fontsize=9, fontweight="bold", va="bottom", ha="left")
    ...
    def on_closing():
        ...
        # Close gpiozero devices (LEDs & Button)
        try:
            GREEN_LED.off()
            RED_LED.off()
            GREEN_LED.close()
